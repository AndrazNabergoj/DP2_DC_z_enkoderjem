...
void ADC_init(void)
{
    ...
    // SOC0 config
    AdcRegs.ADCSOC0CTL.bit.CHSEL = 0x6;     //set SOC0 channel select to ADCINA0
    AdcRegs.ADCSOC0CTL.bit.TRIGSEL = 0x05;  //set SOC0 to start trigger on EPWM1A
    AdcRegs.ADCSOC0CTL.bit.ACQPS = 0x15;       //set SOC0 S/H Window to 7 ADC Clock Cycles,

    // SOC1 config
    AdcRegs.ADCSOC1CTL.bit.CHSEL = 0x6;     //set SOC1 channel select to ADCINA2
    AdcRegs.ADCSOC1CTL.bit.TRIGSEL = 0x05;  //set SOC1 to start trigger on EPWM1A
    AdcRegs.ADCSOC1CTL.bit.ACQPS = 0x15;       //set SOC1 S/H Window to 7 ADC Clock Cycles,

    // SOC2 config
    AdcRegs.ADCSOC2CTL.bit.CHSEL = 0x6;     //set SOC0 channel select to ADCINA0
    AdcRegs.ADCSOC2CTL.bit.TRIGSEL = 0x06;  //set SOC0 to start trigger on EPWM1A
    AdcRegs.ADCSOC2CTL.bit.ACQPS = 0x15;       //set SOC0 S/H Window to 7 ADC Clock Cycles,

    // SOC3 config
    AdcRegs.ADCSOC3CTL.bit.CHSEL = 0x6;     //set SOC1 channel select to ADCINA2
    AdcRegs.ADCSOC3CTL.bit.TRIGSEL = 0x06;  //set SOC1 to start trigger on EPWM1A
    AdcRegs.ADCSOC3CTL.bit.ACQPS = 0x15;       //set SOC1 S/H Window to 7 ADC Clock Cycles,
